...
export default function TodayTasksScreen() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const raw = await AsyncStorage.getItem('prefs_today');
        if (raw) {
  ...
  // Persist preferences debounce
  useEffect(() => {
    const h = setTimeout(() => {
      AsyncStorage.setItem('prefs_today', JSON.stringify({ selectedCategories, sortAsc })).catch(()=>{});
    }, 250);
    return () => clearTimeout(h);
  }, [selectedCategories, sortAsc]);
  const [recentApprovedIds, setRecentApprovedIds] = useState<string[]>([]);

  // Detect tasks that just turned approved (from pending) to highlight briefly
  ...
  useEffect(() => {
    ...
    tasks.forEach(t => {
      ...
      if (prev === 'pending' && t.status === 'approved') {
        newlyApproved.push(t.id);
      }
    });
  ...
  const todayTasks = useMemo(() => {
    let filtered = tasks.filter(task => {
      const allowed = task.status === 'approved' || task.status === 'completed' || (task.status === 'pending' && task.createdBy === user?.uid);
      if (!allowed) return false;
      if (task.date) {
        try { if (!isToday(parseISO(task.date))) return false; } catch { return false; }
      } else {
        // tarefas sem data também contam como hoje
      }
  ...
  <CategoryBar selectedIds={selectedCategories} onChange={setSelectedCategories} />
      <FlatList
        ...
        style={styles.list}
        ListEmptyComponent={<Text style={styles.emptyText}>Nenhuma tarefa para hoje!</Text>}
      />
    </View>
  );
}
