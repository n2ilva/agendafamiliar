...
export const Header: React.FC<HeaderProps> = ({ 
  ...
  const markedDates = useMemo(() => {
    ...
    // Marcar dia de hoje com background na mesma cor da borda
    const today = new Date();
    const ymd = `${today.getFullYear()}-${String(today.getMonth()+1).padStart(2,'0')}-${String(today.getDate()).padStart(2,'0')}`;
    const existingStyle = map[ymd]?.customStyles || {};
    
    // Se já existe uma tarefa no dia de hoje, usar a cor da borda como background
    if (map[ymd] && existingStyle.container?.borderColor) {
      ...
      // Se não tem tarefa, marcar com background azul
      map[ymd] = {
        customStyles: {
          container: {
