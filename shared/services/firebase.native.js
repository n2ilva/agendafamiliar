...
if (Platform.OS === 'web') {
  // No web, usamos localStorage/sessionStorage
...
class FirebaseService {
  constructor() {
    ...
    // Inicializa o Auth dependendo da plataforma
    if (Platform.OS === 'web') {
      // No web, usa getAuth sem persistência específica
      this.auth = getAuth(this.app);
    } else {
      // No mobile, usa persistência com AsyncStorage
      try {
  ...
  async signUpWithEmail(email, password) {
    try {
      ...
      // Atualiza currentUser imediatamente após signUp
      this.currentUser = result.user;
      return result.user;
    } catch (error) {
  ...
  async syncHistory(userId, history) {
    try {
      ...
      toDelete.forEach(itemId => {
        operations.push(deleteDoc(doc(historyCollectionRef, String(item.id))));
      });
